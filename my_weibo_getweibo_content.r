...
# 分词
require(rmmseg4j)
weibo_data <- gsub('(^[^(//)]*)//.*$','\\1',weibo_data)
weibo_data <- weibo_data[weibo_data != '']
f_cut <- function(x){
...
words_df4 <- words_df3[order(-words_df3$words_freq2), ][1:100, ]
...
wordcloud(words_df3$words_names, words_df3$words_freq2, min.freq=0, scale=c(6,1), 
          max.words=100, random.order=F, colors=terrain.colors(100,1))
dev.off()
